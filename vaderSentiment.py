...
def make_lex_dict(f):
    return dict(map(lambda w_m: (w_m[0], float(w_m[1])), [wmsr.strip().split('\t')[0:2] for wmsr in open(f)]))
...
if __name__ == '__main__':
    # --- examples -------
    sentences = [
                "Abhishek is smart, handsome, and funny.",       # positive sentence example
                "Aniruddh is smart, handsome, and funny!",       # punctuation emphasis handled correctly (sentiment intensity adjusted)
                "Shreyas is very smart, handsome, and funny.",  # booster words handled correctly (sentiment intensity adjusted)
                "Samruddh is VERY SMART, handsome, and FUNNY.",  # emphasis for ALLCAPS handled
                "Abhishek javali is VERY SMART, handsome, and FUNNY!!!",# combination of signals - VADER appropriately adjusts intensity
                "Dhanush is VERY SMART, really handsome, and INCREDIBLY FUNNY!!!",# booster words & punctuation make this close to ceiling for score
